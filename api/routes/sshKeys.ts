 ...
 */
async function getLinodeToken(): Promise<string | undefined> {
  try {
    console.log('🔍 Fetching Linode token from database...');
    ...
    if (result.rows.length === 0) {
      console.warn('⚠️  No active Linode provider configured.');
      return undefined;
    }

    try {
      const decrypted = decryptSecret(result.rows[0].api_key_encrypted);
      if (!decrypted || decrypted.trim().length === 0) {
        console.error('❌ Decrypted Linode token is empty or invalid');
        return undefined;
      }
      ...
      const maskedToken =
        decrypted.length > 8
          ...
          : '****';
      console.log(`✅ Successfully decrypted Linode token: ${maskedToken}`);
      return decrypted;
    } catch (error: any) {
      console.error('❌ Failed to decrypt Linode API token:', {
        error: error.message,
        stack: error.stack,
      });
      return undefined;
    ...
    console.error('❌ Error fetching Linode token from database:', {
      error: error.message,
      stack: error.stack,
    });
    return undefined;
...
router.post('/', [
  ...
  try {
    ...
    // Get provider API token
    const linodeToken = await getLinodeToken();
    ...
    if (linodeToken) {
      const tokenPreview = linodeToken.length > 8
        ? `${linodeToken.substring(0, 4)}...${linodeToken.substring(linodeToken.length - 4)}`
        : '****';
      ...
      try {
        const linodeKey = await withRetry(
          () => linodeService.createSSHKey(linodeToken, providerLabel, publicKey),
          { maxRetries: 2 }
        );
...
router.delete('/:keyId', async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    if (key.linode_key_id && linodeToken) {
      ...
      try {
        await withRetry(
          () => linodeService.deleteSSHKey(linodeToken, key.linode_key_id),
          { maxRetries: 2 }
        );
