...
function parseRateLimitConfig(): RateLimitConfig {
    const config: RateLimitConfig = {
        ...
        anonymousMaxRequests: parseInt(
            process.env.RATE_LIMIT_ANONYMOUS_MAX || "200",
            10,
        ),
    ...
    if (config.authenticatedMaxRequests < 1) {
        validationErrors.push(
            "RATE_LIMIT_AUTHENTICATED_MAX must be at least 1",
        );
    }
    ...
    if (validationErrors.length > 0) {
        ...
        if (config.authenticatedMaxRequests < 1)
            config.authenticatedMaxRequests = 500;
...
function getConfig(): Config {
    ...
    console.log("Config loaded:", {
        ...
        hasSmtpCredentials:
            !!config.SMTP2GO_USERNAME && !!config.SMTP2GO_PASSWORD,
...
export function validateRateLimitConfig(rateLimitConfig: RateLimitConfig): {
    ...
    if (
        rateLimitConfig.adminMaxRequests <
        rateLimitConfig.authenticatedMaxRequests
    ) {
        errors.push(
