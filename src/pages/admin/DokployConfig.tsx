...
import type { DokployConfigRequest } from '@/types/containers'
...
export default function DokployConfig() {
  ...
  const { data: configResponse, isLoading } = useQuery({
    queryKey: ['admin', 'dokploy-config'],
    queryFn: containerService.getDokployConfig
  })

  const config = configResponse?.config

  const saveConfigMutation = useMutation({
    mutationFn: containerService.updateDokployConfig,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin', 'dokploy-config'] })
      setHasChanges(false)
      // Clear the API key field after successful save for security
      setFormData(prev => ({ ...prev, apiKey: '' }))
      toast.success('Dokploy configuration saved successfully')
    },
    onError: (error: any) => {
  ...
  const testConnectionMutation = useMutation({
    mutationFn: containerService.testDokployConnection,
    onSuccess: async (result) => {
      // Invalidate and refetch to get updated connection status
      await queryClient.refetchQueries({ queryKey: ['admin', 'dokploy-config'] })
      setTestResult({
        status: 'success',
        message: result.message || 'Connection test successful'
      })
      toast.success('Dokploy connection test successful')
    },
  ...
  const handleSaveConfig = () => {
    ...
    const configData: DokployConfigRequest = {
      ...
      <div className="relative overflow-hidden rounded-xl border bg-gradient-to-br from-card via-card to-muted/20 p-6 md:p-8">
        <div className="relative z-10">
          ...
          <p className="mt-2 max-w-2xl text-muted-foreground">
            Configure connection to your Dokploy instance for container management
          </p>
        </div>
              ...
              {isLoading ? (
                ...
                <>
                  <div className="space-y-2">
                    <Label htmlFor="apiUrl">Dokploy API URL</Label>
                    <Input
                      id="apiUrl"
                      type="url"
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      placeholder={config?.hasApiKey ? "Enter new API key (leave blank to keep current)" : "Enter your Dokploy API key"}
                    />
                    {formErrors.apiKey && (
          ...
          <Card className="mt-6">
            ...
            <CardContent className="space-y-3">
              <div className="text-sm space-y-2">
                <p className="font-medium">To configure Dokploy:</p>
                <ol className="list-decimal list-inside space-y-1 text-muted-foreground">
                  <li>Log into your Dokploy instance</li>
                  <li>Navigate to Settings → User → Your Admin User → API Keys</li>
                  <li>Copy The API Key And Paste It Above</li>
                  <li>Enter Dokploy URL (e.g., https://panel.example.com)</li>
